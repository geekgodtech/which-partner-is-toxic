...
for arb_file in arb_files:
    ...
    # Find the position to insert (after @discordStep5Description closing)
    # Look for the pattern: },\n  "gotIt":
    insert_marker = '"@discordStep5Description":'

    if insert_marker not in content:
        print(f'WARNING: Could not find {insert_marker} in {arb_file}')
        continue

    # Find the closing of @discordStep5Description and gotIt
    # We want to insert before gotIt
    gotit_marker = '"gotIt":'

    if gotit_marker not in content:
        print(f'WARNING: Could not find {gotit_marker} in {arb_file}')
        continue
  ...
  '''
    ...
    # Insert before gotIt
    content = content.replace(f'  {gotit_marker}', new_entry + gotit_marker)
