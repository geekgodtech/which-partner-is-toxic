...
# For each language, find its limit_reached line (or last key line) and append new keys after it.
# Each lang section closes with:  \n  },\n
# Strategy: find the lang block, find the last }, that closes it, insert before it.
...
LANG_DATA = {
  'es': {
    ...
    'use_own_desc': 'El archivo Sample.txt es solo un ejemplo. Puedes editarlo o reemplazarlo con cualquier conversación real exportada de tu teléfono.',
  ...
  'fr': {
    ...
    'use_own_desc': 'Le fichier Sample.txt est juste un exemple. Modifiez-le ou remplacez son contenu par une vraie conversation exportée de votre téléphone.',
  ...
  'de': {
    ...
    'use_own_desc': 'Die Sample.txt-Datei ist nur ein Beispiel. Ersetzen Sie sie durch ein echtes exportiertes Gespräch.',
  ...
  'pt': {
    ...
    'use_own_desc': 'O ficheiro Sample.txt é apenas um exemplo. Substitua-o por uma conversa real exportada do seu telemóvel.',
...
for lang_code, translations in LANG_DATA.items():
    ...
    if lang_start == -1:
        print(f'{lang_code}: NOT FOUND in TRANSLATIONS')
        continue

    # Find closing }  of this lang block — it's '  },' followed by newline
    # Search from lang_start forward for the pattern \n  },
    close_match = re.search(r'\n  \},', content[lang_start+5:])
    if not close_match:
        print(f'{lang_code}: close not found')
        continue
    close_pos = lang_start + 5 + close_match.start()  # position of \n  },

    # Build the keys to insert (all keys in translations dict)
    new_keys_str = ''
    for k, v in translations.items():
        new_keys_str += f"    {k}: '{v}',\n"

    # Insert before the closing },
    content = content[:close_pos] + '\n' + new_keys_str + content[close_pos:]
    print(f'{lang_code}: {len(translations)} keys injected')
